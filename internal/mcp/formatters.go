...
func (s *Server) formatRepositoryResponse(result *marketplace.RepositoryResponse) string {
	...
	var sb strings.Builder
	sb.WriteString(fmt.Sprintf("Found %d repositories (page %d, showing %d):\n\n", result.Count, result.Page+1, len(result.Repositories)))
	...
	if result.Count > len(result.Repositories) {
		remaining := result.Count - ((result.Page) * result.Size) - len(result.Repositories)
		if remaining > 0 {
			sb.WriteString(fmt.Sprintf("**Note:** There are %d more repositories. Use `page: %d` to see the next page.\n", remaining, result.Page+1))
		}
	}
